...
(function () {
  ...
  function hasInjectionMarker(text) {
    return text.includes('[USER CONTEXT START]') || text.includes('[USER CONTEXT END]') || text.includes('[Memory:');
  }
  ...
  async function injectMemory(el) {
    ...
    try {
      ...
      // Step 4: System prompt (if no task mode active and token saving enabled)
      if (settings.tokenSavingPrompt && !overrides.skipSystemPrompt && !overrides.taskMode && !settings.activeMode) {
      ...
      showToast('Context injected ✓', 'info');
      return true;
    } catch (err) {
