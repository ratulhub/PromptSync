...
const FloatingPanel = (() => {
  ...
  function injectPanel() {
    ...
    // Populate roles async
    populateRoles(modeSelect);

    modeSelect.addEventListener('change', async () => {
      try {
        await MemoryStorage.setActiveRole(modeSelect.value);
        showMiniToast(modeSelect.value ? `Role: ${modeSelect.value}` : 'Default role');
      } catch {}
    });
    ...
    posRow.className = 'asm-fp__row';
    const posLabel = document.createElement('span');
    posLabel.textContent = 'Inject Pos';
    const posSelect = document.createElement('select');
    posSelect.className = 'asm-fp__select';
    posSelect.id = 'asm-fp-pos';
    posSelect.innerHTML = '<option value="TOP">Top</option><option value="BOTTOM">Bottom</option>';
    
    posSelect.addEventListener('change', async () => {
      try {
        const s = await MemoryStorage.getSettings();
        s.injectionPosition = posSelect.value;
        await MemoryStorage.saveSettings(s);
        showMiniToast(`Position: ${posSelect.value}`);
      } catch {}
    });
  ...
  async function populateRoles(select) {
    try {
      ...
      for (const [key, role] of Object.entries(roles)) {
        ...
        opt.value = key;
        opt.textContent = role.name || key;
        select.appendChild(opt);
      }
      if (settings.activeRole) select.value = settings.activeRole;
    } catch {}
  }
  ...
  async function loadInitialState() {
    try {
      ...
      const posSelect = document.getElementById('asm-fp-pos');
      if (posSelect && s.injectionPosition) posSelect.value = s.injectionPosition;
    } catch {}
  }
