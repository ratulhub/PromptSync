...
const ContextEngine = (() => {
  ...
  async function buildWithTemplate(userText, profile, items, settings, overrides, template) {
    ...
    if (output === '[USER CONTEXT START]\n[USER CONTEXT END]') output = '';
    else output = output + '\n\n';

    // ── Build Role and Style injection ──
    let roleInjectionStr = '';
    const roleMode = settings.roleInjectionMode || 'AUTO';
    let targetRoleName = overrides.taskRole;

    if (!targetRoleName && roleMode === 'AUTO') {
      targetRoleName = settings.activeRole;
    }

    if (roleMode !== 'OFF' && targetRoleName && typeof MemoryStorage !== 'undefined') {
      try {
        const roles = await MemoryStorage.getRoles();
        const targetRole = roles[targetRoleName];
        if (targetRole && targetRole.instruction) {
          roleInjectionStr = `[ROLE]\n${targetRole.instruction}\n[/ROLE]\n\n`;
          if (settings.enableStyleBlock !== false) {
            const styleInstr = settings.styleInstructions || '* Be direct\n* Avoid unnecessary explanation\n* Focus on quality output';
            roleInjectionStr += `[STYLE]\n${styleInstr}\n[/STYLE]\n\n`;
          }
        }
      } catch (e) {
    ...
    const finalOutput = (output + roleInjectionStr).trim();
