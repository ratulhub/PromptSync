...
/* ═══════════════════════
   MODES TAB
═══════════════════════ */
let modesTabBound = false;
async function initModesTab() {
  await renderModes();
  await refreshActiveModeSelector();
  await renderCustomCommands();

  // Bind event listeners only once
  if (modesTabBound) return;
  modesTabBound = true;

  $('#js-mode-save').addEventListener('click', async () => {
    const name = $('#js-mode-name').value.trim();
    const inst = $('#js-mode-instruction').value.trim();
    if (!name || !inst) { showPopupToast('Name and instruction required'); return; }
    const key = name.toLowerCase().replace(/\s+/g, '_');
    const modes = await MemoryStorage.getTaskModes();
    modes[key] = { name, instruction: inst, builtin: false };
    await MemoryStorage.saveTaskModes(modes);
    $('#js-mode-name').value = ''; $('#js-mode-instruction').value = '';
    await renderModes(); await refreshActiveModeSelector();
    showPopupToast(`Mode "${name}" created`);
  });

  // Active mode selector
  const activeSel = $('#js-active-mode');
  activeSel.addEventListener('change', async () => {
    await MemoryStorage.setActiveMode(activeSel.value);
  ...
  $('#js-cmd-save').addEventListener('click', async () => {
    ...
    const cmd = {
      ...
      actions: {
        ...
        taskMode: $('#js-cmd-mode').value || undefined,
    ...
    $('#js-cmd-disable-mem').checked=false; $('#js-cmd-skip-sys').checked=false; $('#js-cmd-mode').value='';
    await renderCustomCommands(); showPopupToast(`Command ${cmd.name} added`);
  });
}

async function refreshActiveModeSelector() {
  const activeSel = $('#js-active-mode');
  const modes = await MemoryStorage.getTaskModes();
  const settings = await MemoryStorage.getSettings();
  ...
  activeSel.value = settings.activeMode || '';
}

async function renderModes() {
  const modes = await MemoryStorage.getTaskModes();
  const list = $('#js-mode-list');
  list.innerHTML = '';
  for (const [key, m] of Object.entries(modes)) {
  ...
  $$('.js-mode-delete', list).forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      const modes = await MemoryStorage.getTaskModes();
      delete modes[key]; await MemoryStorage.saveTaskModes(modes);
      await renderModes(); showPopupToast('Mode deleted');
      const s = await MemoryStorage.getSettings();
      if (s.activeMode === key) { s.activeMode = ''; await MemoryStorage.saveSettings(s); }
      await refreshActiveModeSelector();
    });
  });
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  await Promise.all([
    ...
    initModesTab(), initSettingsTab(), initHelpTab(), updateStatusBadge()
  ]);
});