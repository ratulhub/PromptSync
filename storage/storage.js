...
const MemoryStorage = (() => {
  ...
  const DEFAULT_MODES = {
    coding: {
      name: 'Coding',
      instruction: 'You are a coding assistant. Give clean, minimal, working code. No explanation unless asked.',
      builtin: true
    },
    study: {
      name: 'Study',
      instruction: 'Explain simply step by step. Use easy words. Give examples.',
      builtin: true
    },
    short: {
      name: 'Short',
      instruction: 'Answer in the shortest possible way. No extra explanation.',
      builtin: true
    }
  };
  ...
  async function getTaskModes() {
    ...
    return (raw && typeof raw === 'object') ? { ...DEFAULT_MODES, ...raw } : { ...DEFAULT_MODES };
  }
  ...
  async function exportData() {
    const [profiles, items, settings, modes, commands, template] = await Promise.all([
      getProfiles(), getItems(), getSettings(), getTaskModes(), getCustomCommands(), getTemplate()
    ]);
  ...
  async function importData(jsonStr) {
    ...
    if (data.modes && typeof data.modes === 'object') {
      payload[KEYS.MODES] = { ...DEFAULT_MODES, ...data.modes };
    }
  ...
  return {
    ...
    MAX_ITEMS, DEFAULT_PROFILE, DEFAULT_TEMPLATE, DEFAULT_MODES
  };
})();