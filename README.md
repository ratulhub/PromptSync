...
2. Open `chrome://extensions` in Chrome (or equivalent in your browser)
...
4. Select the `manifest.json` file inside the project folder (or your built `.zip` file)
5. **Important:** Do NOT use `about:addons` "Install Add-on From File" for local testing. Firefox will incorrectly say the add-on is "corrupt" because it is unsigned by Mozilla. Always use `about:debugging`.
